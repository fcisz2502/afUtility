...
# -----------------------------------------------------
def local_vs_mongo(machine_id, instrument_list, strategy, mail_receivers=[cwhEmail]):
...
#        afternoon_end_time = time(15)
        ...
        if datetime.now().weekday() < 5:    
            ...
            if datetime.now().time() > time(23, 30):
                print('--1.1.1 after 2330--')
                # get the open price and use it as highPrice and lowPrice            
                try:
                    open_ticks = pd.DataFrame()           
                    for row1 in collection.find({'datetime': {'$gte': datetime.combine(datetime.now().date(), time(20, 59)), 
                                                                  '$lt': datetime.combine(datetime.now().date(), time(21))}}, 
                                           projection=['datetime', 'lastPrice', 'highPrice', 'lowPrice']):
                        open_ticks = open_ticks.append(pd.DataFrame(row1, index=[0]))
                        
                    open_ticks.reset_index(drop=True, inplace=True)
                        
                    dayHigh = open_ticks.loc[0, 'lastPrice']   
                    dayLow = open_ticks.loc[0, 'lastPrice'] 
                
                except:
                    pass
    
                # start time and end time of last trading section
                dt = datetime.combine(datetime.now().date(), time(20, 59))
                dt1 = datetime.combine(datetime.now().date(), time(23, 30, 2))
    #            dt2 = datetime.combine(datetime.now().date(), time(23))    
    ...
    #            dt2 = datetime.combine(datetime.now().date(), time(15))
                ...
                # get last night section highPrice and lowPrice            
                if datetime.now().weekday() == 0:   
                    print('--1.1.31 today is Monday--')
                    try:
                        night_ticks = pd.DataFrame()           
                        for row3 in collection.find({'datetime': {'$gte': datetime.combine(datetime.now().date()-pd.Timedelta('3 days'), time(22, 59, 58)), 
                                                                      '$lt': datetime.combine(datetime.now().date()-pd.Timedelta('3 days'), time(23,30,2))}}, 
                                               projection=['datetime', 'lastPrice', 'highPrice', 'lowPrice']):
                            night_ticks = night_ticks.append(pd.DataFrame(row3, index=[0]))
                            
                        night_ticks.reset_index(drop=True, inplace=True) 
        
                        dayHigh = night_ticks.loc[len(night_ticks)-1, 'highPrice']
                        dayLow = night_ticks.loc[len(night_ticks)-1, 'lowPrice']
                    ...
                    print('--1.1.32 today is not Monday--')
                    try:
                        night_ticks = pd.DataFrame()           
                        for row3 in collection.find({'datetime': {'$gte': datetime.combine(datetime.now().date()-pd.Timedelta('1 day'), time(22, 59, 58)), 
                                                                      '$lt': datetime.combine(datetime.now().date()-pd.Timedelta('1 day'), time(23,30,2))}}, 
                                               projection=['datetime', 'lastPrice', 'highPrice', 'lowPrice']):
                            night_ticks = night_ticks.append(pd.DataFrame(row3, index=[0]))
    ...
    #                try:
    #                    night_ticks = pd.DataFrame()           
    #                    for row3 in collection.find({'datetime': {'$gte': datetime.combine(datetime.now().date()-pd.Timedelta('2 days'), time(22, 59, 58)), 
    #                                                                  '$lt': datetime.combine(datetime.now().date()-pd.Timedelta('2 days'), time(23,30,2))}}, 
    #                                           projection=['datetime', 'lastPrice', 'highPrice', 'lowPrice']):
    #                        night_ticks = night_ticks.append(pd.DataFrame(row3, index=[0]))
    #                        
    #                    night_ticks.reset_index(drop=True, inplace=True) 
    ...
    #                                         ' ', mail_receivers)
                    ...
                    print('now is weekend, what do you want?')
    ...
    #            dt2 = datetime.combine(datetime.now().date(), time(13, 30))
            ...
            # Monday, fetch last trading night data, fn = Friday night, ln = last night
            elif datetime.now().time() < time(11,30):
                print('--1.1.4 before 1130--')
                # last friday night section highPrice and lowPrice            
                if datetime.now().weekday() == 0:
                    print('--1.1.41 today is Monday--')
                    try:
                        fn_open_ticks = pd.DataFrame()           
                        for row4 in collection.find({'datetime': {'$gte': datetime.combine(datetime.now().date()-pd.Timedelta('3 days'), time(20, 59)), 
                                                                      '$lt': datetime.combine(datetime.now().date()-pd.Timedelta('3 days'), time(21))}}, 
                                               projection=['datetime', 'lastPrice', 'highPrice', 'lowPrice']):
                        ...
                        send_message(machine_id + instrument, ' ----cannot fetch dayHigh or dayLow from mongo----',
                                             ' ', mail_receivers)
                    dt = datetime.combine(datetime.now().date()-pd.Timedelta('3 days'), time(20, 59))
                    dt1 = datetime.combine(datetime.now().date()-pd.Timedelta('3 days'), time(23, 30, 2))
                
                elif datetime.now().weekday() in [1, 2, 3, 4]:
                    print('--1.1.42 today is not Monday--')
                    try:
                        ln_open_ticks = pd.DataFrame()           
                        for row4 in collection.find({'datetime': {'$gte': datetime.combine(datetime.now().date()-pd.Timedelta('1 days'), time(20, 59)), 
                                                                      '$lt': datetime.combine(datetime.now().date()-pd.Timedelta('1 days'), time(21))}}, 
                                               projection=['datetime', 'lastPrice', 'highPrice', 'lowPrice']):
                            ln_open_ticks = ln_open_ticks.append(pd.DataFrame(row4, index=[0]))
                            
                        ln_open_ticks.reset_index(drop=True, inplace=True)
                            
                        dayHigh = ln_open_ticks.loc[0, 'lastPrice']   
                        dayLow = ln_open_ticks.loc[0, 'lastPrice']          
                    
                    except:
                        send_message(machine_id + instrument, ' ----cannot fetch dayHigh or dayLow from mongo----',
                                             ' ', mail_receivers)
                                             
                    dt = datetime.combine(datetime.now().date()-pd.Timedelta('1 day'), time(20, 59))
                    dt1 = datetime.combine(datetime.now().date()-pd.Timedelta('1 day'), time(23, 30, 2))    
                
    #            elif datetime.now().weekday() == 6:
    #                print('seriously! on weekend?')
    #                try:
    #                    ln_open_ticks = pd.DataFrame()           
    #                    for row4 in collection.find({'datetime': {'$gte': datetime.combine(datetime.now().date()-pd.Timedelta('1 days'), time(20, 59)), 
    #                                                                  '$lt': datetime.combine(datetime.now().date()-pd.Timedelta('1 days'), time(21))}}, 
...
#        elif datetime.now().time() > time(15):
        elif dftick.loc[0, 'datetime'].time() > time(13, 20):
...
#        filePath = 'C:\\vnpy-1.9.2\\examples\\'
        filePath = os.path.join('c:\\', 'vnpy-1.9.2', 'examples')
...
#                doc = filePath + folder + '\\' + instrument + '_df_60m.csv'   
        ...
        #                             mail_receivers) 
                ...
                elif symbol in ['rb', 'ru', 'CF', 'bu', 'p', 'm']:
...
if __name__ == "__main__":
    machine_id = 'VM2 '
    # local_vs_mongo(machine_id, ['rb1910', 'ru2001', 'i2001'], 'Probot', [cwhEmail])
    # local_vs_mongo(machine_id, ['rb1910'], 'Probot')
#     local_vs_mongo(machine_id, ['i2001'], 'Probot')
    # local_vs_mongo(machine_id, ['rb1910'], 'Probot')
    local_vs_mongo(machine_id, [], 'RBT')
